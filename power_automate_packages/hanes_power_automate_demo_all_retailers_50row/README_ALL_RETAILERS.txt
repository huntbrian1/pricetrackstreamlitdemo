...
- Dollar General and JCPenney wait 5 to 9 seconds between rows, restart/rest every 40 rows, and take a longer rest every 100 rows.
- Non-Target browser retailers are capped at 150 attempted rows per retailer per run by default.
- If a true block page is detected for Dollar General or JCPenney, the runner saves immediately and skips the rest of that retailer for the current run.
...
2. Local Playwright runs Target/Dollar General/JCPenney.
