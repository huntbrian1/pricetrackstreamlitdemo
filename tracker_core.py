...
BASE_COLUMNS = ["retailer", "brand", "color", "size", "link", "title", "last_seen"]
RUN_COLUMNS = ["scrape_status", "scrape_error", "price_source", "raw_price_text"]
...
def normalize_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in BASE_COLUMNS + RUN_COLUMNS:
    ...
    ordered = BASE_COLUMNS + price_columns(df) + RUN_COLUMNS
    other_cols = [col for col in df.columns if col not in ordered]
...
def merge_results_into_master(
    ...
    for result in results:
        ...
        master.at[idx, "last_seen"] = now.isoformat()
        master.at[idx, price_col] = "" if result.price is None else result.price
        master.at[idx, "scrape_status"] = result.status
        master.at[idx, "scrape_error"] = result.error
        master.at[idx, "price_source"] = result.source
        master.at[idx, "raw_price_text"] = result.raw_price_text
